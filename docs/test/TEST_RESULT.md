...
## 추가 검증 — 야간 근무 자정 분할 (2026-05-26 추가)

`splitByMidnight()` 유틸 신설 + payroll·UI 전반에 적용 (정책: 자정 기준 시간 분할, 일수 2일).

### 야간 근무 자정 분할 (9/9)
- ✅ 야간 1건(5/26 22:00 ~ 5/27 06:00) → segments 2개 (120분 + 360분)
- ✅ GET attendance 응답에 `segments[]` 포함
- ✅ 월 경계 근무(5/31 22:00 ~ 6/1 06:00) — 5월/6월 양쪽 조회에서 매칭
- ✅ 5월 payroll: 월 경계 근무는 5월 segment(120분)만 카운트
- ✅ 6월 payroll: 6월 segment(360분)만 카운트 (중복 없음)
- ✅ 5월 unique 근무일: 5/26·5/27·5/28·5/29·5/31 = **5일** (야간 분할 반영)
- ✅ 주휴수당 segments 기준으로 정확 계산
- ✅ ManagerPayrollPage / openPayslip PDF / downloadPayslipCsv 모두 segment 행 출력
- ✅ PersonalPage 일수·시간 segments 기반으로 산정

### 핵심 동작 변경
| 기존 | 신규 |
|---|---|
| 출근일 기준 1일 카운트 | 출근일·자정후 일자 각각 카운트 (1건 → 최대 2일) |
| `clock_in 날짜`에 시간 통째 합산 | 자정 분할 후 각 일자에 시간 배분 |
| 명세서 PDF/CSV 한 줄 | segment 단위 여러 줄 |
| 월 조회 `clock_in LIKE prefix` | `clock_in OR clock_out LIKE prefix` (월 경계 누락 방지) |

---

## 최종 판정
✅ **자동 테스트 57/57 통과** — 백엔드/데이터 무결성/마이그레이션/마스터 토글/야간 분할 검증 완료
