...
# - 환경변수 없으면 graceful fallback (서버만 시작, 백업 비활성화)
set -e
...
if [ -n "$R2_BUCKET" ] && [ -n "$R2_ACCESS_KEY_ID" ] && [ -n "$R2_SECRET_ACCESS_KEY" ] && [ -n "$R2_ENDPOINT" ]; then
  ...
  litestream restore -if-replica-exists -config /app/litestream.yml "$DB" || echo "[start] restore 실패 또는 backup 없음 — 기존 로컬 DB 사용"
  echo "[start] Litestream replicate + node 서버 시작"
  exec litestream replicate -config /app/litestream.yml -exec "node /app/server/dist/index.js"
else
