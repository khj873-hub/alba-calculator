...
export default async function businessesRoutes(app: FastifyInstance) {
  ...
  app.get('/api/businesses', async () => {
    return db.prepare('SELECT id, slug, name, created_at, lat, lng, radius_meters, home_mode, leave_pay_calc_mode, weekly_holiday_includes_leave FROM businesses ORDER BY created_at DESC').all()
  })

  // 사업장 존재 확인 (PIN 미포함)
  app.get<{ Params: { slug: string } }>(
    '/api/businesses/:slug', async (req, reply) => {
      const biz = db.prepare('SELECT id, slug, name, created_at, lat, lng, radius_meters, home_mode, leave_pay_calc_mode, weekly_holiday_includes_leave FROM businesses WHERE slug = ?').get(req.params.slug)
  ...
  // 휴가 정책 변경 (관리자 세션 인증)
  app.patch<{ Params: { slug: string }; Body: { leave_pay_calc_mode?: '8hours' | 'avg_workhours'; weekly_holiday_includes_leave?: boolean } }>(
    '/api/businesses/:slug/leave-policy', { preHandler: requireManagerAuth }, async (req, reply) => {
      const { leave_pay_calc_mode, weekly_holiday_includes_leave } = req.body
      ...
      const updated = db.prepare('SELECT leave_pay_calc_mode, weekly_holiday_includes_leave FROM businesses WHERE slug = ?').get(req.params.slug)
